...
private:
	...
	void __resizeText( int nInc )
	{
		if ( nInc + _M_daTrie._M_n_offset >= _M_daTrie._M_n_text )
		{
			...
			__charwt * p	= (__charwt *)realloc( _M_daTrie._M_p_text, __nsize * sizeof(__charwt) );
			if ( p == NULL )
			{
				__nsize = ( _M_daTrie._M_n_offset + nInc + 1023 ) & ~1023;
				p = (__charwt *)realloc( _M_daTrie._M_p_text, __nsize * sizeof(__charwt) );
				assert( p != NULL );
			}
	...
	void __setText( int nIdx, __charwt *__p, int __len )
	{
		...
		memcpy( _M_daTrie._M_p_text + _M_daTrie._M_n_offset, __p, __len * sizeof(__charwt) );
	...
	int __addChr( int parent, __charwt c )
	{
...
private:
	...
	int __dynac_load( const char * sIndexName )
	{
		...
		if ( INVALID_HANDLE_VALUE != hFile )
		{
			...
			__daHead._M_p_text = (__charwt *)malloc( sizeof( __charwt ) * __daHead._M_n_text );
			...
			//	text
			ReadFile( hFile, __daHead._M_p_text, sizeof( __charwt ) * __daHead._M_n_text, &dwDummy, NULL );
			//i = fread( __daHead._M_p_text, sizeof(char), sizeof( __charwt ) * __daHead._M_n_text, fp );
			if ( dwDummy != (int)(sizeof( __charwt ) * __daHead._M_n_text ) )
			{
				CloseHandle( hFile );
				return 0;
			}
...
/*
	int __static_load( char * sIndexName )
	{
		...
		__daHead._M_p_text = (__charwt *) ( _M_p_map + sizeof( daTrieHead ) + sizeof( daElm ) * __daHead._M_n_elm + sizeof( wordProp ) * __daHead._M_n_prop );
...
public:
	...
	bool prefixSrch( __charwt *sText, _Tp **ppProp = NULL, int *pId = NULL )
	{
	...
	int prefixMatch( __charwt *sText, _Tp ** ppProp = NULL, int *pId = NULL)
	{
		int left,next,parent = 1;
		int acceptPOINT = 0;
		__charwt *p, *pOK;
	...
	//	前匹配
	//
	int prefixMatch( __charwt *sText, int nLen, _Tp ** ppProp = NULL, int *pId = NULL )
	{
		INT left, next, parent = 1;
		INT acceptPOINT = 0;
		__charwt * p, * q, * pOK;
	...
	// -1:	ERROR
	//0-N:	WORDID
	int insert( __charwt *sWord, const _Tp &iProp = 0 )
	{
		int i, next, parent = 1;
		bool bInsert = false;
		__charwt c;
	...
	bool setProp( __charwt *sWord, const _Tp &iProp )
	{
		_Tp *pProp;
	...
	bool getProp( __charwt *sWord, _Tp &iProp )
	{
		_Tp *pProp;
		bool bSuc = prefixSrch( sWord, &pProp );
	...
	//	for win32
	//	保存词典到文件
	int saveDict( const char * sIndexName )
	{
		int i;
		int len;
		HANDLE hFile	= NULL;
		...
		if ( INVALID_HANDLE_VALUE != hFile)
		{
			...
			//	_KEYBUF
			len = sizeof( __charwt ) * __daHead._M_n_text;
			dwDummy	= 0;
