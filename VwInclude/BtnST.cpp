...
CButtonST::CButtonST()
{
	...
	//	文字距离左边的距离
	m_uTextPaddingLeft	= 0;

	//	是否发送鼠标点击命令消息
	m_bIsSendCommandMsg	= FALSE;

	//	标记按钮曾经工作过
	m_bIsButtonWorked	= FALSE;

	//	当鼠标点了以后，会向下抖动一下
	m_bIsTrembleButton	= TRUE;
...
void CButtonST::OnKillFocus( CWnd * pNewWnd )
{
	...
	if ( ! m_bIsPressed )
	{
		...
		if ( hParent )
		{
			::PostMessage( hParent, BUTTONST_MSG_KILLFOCUS_NOTIFY, pNewWnd ? (WPARAM)pNewWnd->m_hWnd : 0, 10000 );
		}
	}
...
{	
	...
	else
	{
		...
		else
		{
			...
			else if ( m_bIsSendCommandMsg )
			{
				::PostMessage( ::GetParent( GetSafeHwnd() ), WM_COMMAND, this->GetDlgCtrlID(), (LPARAM)NULL);
			}	
		} // else
	} // else
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		*/
		...
		pDC->DrawState(captionRect.TopLeft(), captionRect.Size(), (LPCTSTR)sTitle, (bIsDisabled ? DSS_DISABLED : DSS_NORMAL), 
						TRUE, 0, (CBrush*)NULL);
		*/
		CRect rcText		= captionRect;
		UINT  uTextFormat	= DT_WORDBREAK | DT_CENTER;

		if ( m_uTextPaddingLeft > 0 )
		{
			rcText.left	= m_uTextPaddingLeft + ( m_bIsPressed ? 1 : 0 );
			uTextFormat	= DT_WORDBREAK | DT_LEFT;
		}

		if (m_bIsDisabled)
		{
			if ( ::GetSysColor( COLOR_3DSHADOW ) == m_crColors[ BTNST_COLOR_FG_DISABLED ] )
			{
				captionRect.OffsetRect(1, 1);
				pDC->SetTextColor(::GetSysColor(COLOR_3DHILIGHT));
				pDC->DrawText( sTitle, -1, rcText, uTextFormat );
			...
			{
				captionRect.OffsetRect(1, 1);
				pDC->SetTextColor( m_crColors[ BTNST_COLOR_FG_DISABLED ] );
				pDC->DrawText( sTitle, -1, rcText, uTextFormat );
	...
	switch (m_byAlign)
	{
		case ST_ALIGN_HORIZ:
			...
			else
			{
				...
				rpImage->left += m_uPadding;
				rpTitle->left += dwWidth + m_uPadding;
			}
		...
		case ST_ALIGN_HORIZ_RIGHT:
			...
			{
				// Image must be placed just inside the focus rect
				rpTitle->right = rpTitle->Width() - dwWidth - m_uPadding;
				rpTitle->left = m_uPadding;
				rpImage->left = rBtn.right - dwWidth - m_uPadding;
				// Center image vertically
				rpImage->top += ((rpImage->Height() - (long)dwHeight)/2);
			}
			break;
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rItem, CRect *rCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if ( m_csBitmaps[2].hBitmap && ! m_bIsCheckBox && bIsPressed )
	{
		byIndex = 2;
	}
	else
...
//			Failed creating mask bitmap.
//
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut, HBITMAP hBitmapDown, COLORREF crTransColorDown )
{
	int	nRetValue;
	BITMAP	csBitmapSize;
...
DWORD CButtonST::SetSendCommandMsg( BOOL bSend /* = TRUE */ )
{
	m_bIsSendCommandMsg = bSend;

	return BTNST_OK;
}

DWORD CButtonST::SetTrembleButton( BOOL bTremble /* = TRUE */ )
{
	m_bIsTrembleButton = bTremble;
...
DWORD CButtonST::SetPadding( UINT uPadding, BOOL bRepaint )
{
	m_uPadding	= uPadding;
...
DWORD CButtonST::SetTextPaddingLeft( UINT uTextPaddingLeft, BOOL bRepaint )
{
	m_uTextPaddingLeft = uTextPaddingLeft;
...
DWORD CButtonST::SetDefaultColors(BOOL bRepaint)
{
	...
	m_crColors[ BTNST_COLOR_FG_DISABLED ]	= ::GetSysColor( COLOR_3DSHADOW );
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetBtnCursor(int nCursorId, BOOL bRepaint /*=TRUE*/ )
{
	HINSTANCE	hInstResource = NULL;
...
DWORD CButtonST::SetBtnCursor( HCURSOR hCursor, BOOL bRepaint /*=TRUE*/ )
{
	m_hCursor = hCursor;
