// CompressFile.cpp: implementation of the CVwZip class.
//
//////////////////////////////////////////////////////////////////////
...
CVwZip::CVwZip( PSP_UNCOMPRESS_CALLBACK pfnDetailInfo ) : m_pfnCallbackDetailInfo( pfnDetailInfo )
{
}
 ...
 *	解压一个 ZIP 文件
 */
BOOL CVwZip::UnZipFiles( LPCSTR lpZipFile, LPCSTR lpUnZipFileDir, BOOL bUnZipSubDir, BOOL bFirstEnter )
{
	/*
 ...
 *	判断文件是否存在
 */ 
BOOL CVwZip::MyFileExists( LPCTSTR lpszFile )
{
	BOOL bRet = FALSE;
 ...
 *	替换字符串中指定的字符为新的字符
 */
BOOL CVwZip::StrReplace( char *lpSrc, char chOld, char chNew )
{
	//
	//	lpSrc	- [in] 指定字符串
	//	chOld	- [in] 指定字符
 ...
 *	安全地创建多层目录
 */
BOOL CVwZip::MultiCreateDirectory( LPCTSTR lpcszDir )
{
	//
	//	lpcszDir	- [in] 需要创建的目录
	//
 ...
 *	解开一个 Zip 压缩包到指定目录下
 */
BOOL CVwZip::UnZip( LPCSTR lpZipFile, LPCSTR lpUnZipFileDir )
{
	//
 ...
 *	...
 */
VOID CVwZip::VerifyZip(int nCount, HZIP hz, LPCTSTR lpszFile)
{
	if ( ! hz )
	{
		return;
