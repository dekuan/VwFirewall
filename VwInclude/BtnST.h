...
//	message
//
#ifndef BUTTONST_MSG_KILLFOCUS_NOTIFY
	#define BUTTONST_MSG_KILLFOCUS_NOTIFY		WM_USER + 10000
#endif
 ...
 *		class CButtonST : public CBitmapButton
 */
class CButtonST : public CBitmapButton
{
public:
	...
	enum
	{
		...
		BTNST_COLOR_FG_DISABLED,	//	Text color when the button is disabled
...
public:
	...
	DWORD SetSendCommandMsg( BOOL bSend = TRUE );
	DWORD SetTrembleButton( BOOL bTremble = TRUE );
	DWORD SetFlat(BOOL bFlat = TRUE, BOOL bRepaint = TRUE);
	DWORD SetAlign(BYTE byAlign, BOOL bRepaint = TRUE);
	DWORD SetTextPaddingLeft( UINT uTextPaddingLeft, BOOL bRepaint = TRUE );
	DWORD SetPadding( UINT uPadding, BOOL bRepaint = TRUE );
...
protected:
	...
	UINT		m_uPadding;			// 整体 Padding
	UINT		m_uTextPaddingLeft;		// 文字距离左边的距离
	BOOL		m_bIsSendCommandMsg;		// 是否发送点击命令消息
	BOOL		m_bIsButtonWorked;		// 标记按钮是否曾经工作过
	BOOL		m_bIsTrembleButton;		// is a trembling button. add by VirtualWall, Inc.
