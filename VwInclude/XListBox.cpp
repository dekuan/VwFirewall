...
int CXListBox::InsertString(int nIndex, LPCTSTR lpszItem)
{
	...
	Color tc = clrBlack;			// to force black-only text
	Color bc = clrWhite;
...
{
	AddLine(CXListBox::clrBlack, CXListBox::clrWhite, lpszItem);
}
...
void CXListBox::AddLine(Color tc, Color bc, LPCTSTR lpszLine)
{
	...
	if (!m_bColor)
	{
		tc = clrBlack;			// to force black-only text
		bc = clrWhite;
	}
...
void CXListBox::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	{
		nFindPos = strLine.Find( "http://" );
		if ( nFindPos >= 0 )
		{
			strUrl = strLine.Mid( nFindPos );
			if ( strUrl.GetLength() )
			{
				nFindPos = strUrl.Find( " " );
				if ( nFindPos > 0 )
				{
					strUrl = strUrl.Left( nFindPos );
				}

				//	打开一个 IE 进程
				LauchIEBrowser( strUrl );
			}
		}
	}
 ...
 *	打开一个 URL
 */
BOOL CXListBox::LauchIEBrowser( LPCTSTR lpcszUrl )
{
	__try
	{
