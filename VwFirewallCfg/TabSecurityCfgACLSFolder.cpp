// TabSecurityCfgACLSFolder.cpp : implementation file
//
...
CTabSecurityCfgACLSFolder::CTabSecurityCfgACLSFolder(CWnd* pParent /*=NULL*/)
	: CDialog(CTabSecurityCfgACLSFolder::IDD, pParent)
{
	//{{AFX_DATA_INIT(CTabSecurityCfgACLSFolder)
...
CTabSecurityCfgACLSFolder::~CTabSecurityCfgACLSFolder()
{
...
void CTabSecurityCfgACLSFolder::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CTabSecurityCfgACLSFolder)
...
BEGIN_MESSAGE_MAP(CTabSecurityCfgACLSFolder, CDialog)
	//{{AFX_MSG_MAP(CTabSecurityCfgACLSFolder)
...
/////////////////////////////////////////////////////////////////////////////
// CTabSecurityCfgACLSFolder message handlers

BOOL CTabSecurityCfgACLSFolder::OnInitDialog() 
{
...
BOOL CTabSecurityCfgACLSFolder::PreTranslateMessage(MSG* pMsg) 
{
...
void CTabSecurityCfgACLSFolder::OnButtonDetect() 
{
	DetectAclsProblems();
...
void CTabSecurityCfgACLSFolder::OnButtonFix() 
{
	if ( ! m_bDoingFix )
	{
		if ( ! m_bFixInProgress )
...
void CTabSecurityCfgACLSFolder::OnButtonRecover() 
{
...
void CTabSecurityCfgACLSFolder::OnRclickListGroup(NMHDR* pNMHDR, LRESULT* pResult) 
{
	BOOL bHasSel = FALSE;
...
void CTabSecurityCfgACLSFolder::OnDblclkListGroup(NMHDR* pNMHDR, LRESULT* pResult) 
{
	OnMenuListDetail();
	*pResult = 0;
}
...
void CTabSecurityCfgACLSFolder::OnMenuListDetail() 
{
	INT nItemSel		= 0;
	INT nIndex;
	...
	if ( p_ListPos )
	{
		...
		if ( nItemSel >=0 && nItemSel < m_listGroup.GetItemCount() )
		{
			...
			if ( nIndex >= 0 && nIndex < m_CfgAclsFolderData.m_vcAclsGroupList.size() )
			{
				pstAclsDataGroup = & m_CfgAclsFolderData.m_vcAclsGroupList.at( nIndex );
				if ( pstAclsDataGroup )
				{
...
VOID CTabSecurityCfgACLSFolder::LoadDataFromCfgFile()
{
	//	...
	m_CfgAclsFolderData.LoadData( m_cVwFirewallConfigFile.m_szCfgAclsFolderData );

	m_proBar.SetRange( 0, m_CfgAclsFolderData.m_vcAclsGroupList.size() );
	m_proBar.SetPos( 0 );
}

VOID CTabSecurityCfgACLSFolder::InitList()
{
	STPARSECFGDATAGROUP * pstAclsDataGroup;
...
//	TCHAR szTemp[ 1024 ];
	...
	for ( m_CfgAclsFolderData.m_it = m_CfgAclsFolderData.m_vcAclsGroupList.begin();
		m_CfgAclsFolderData.m_vcAclsGroupList.end() != m_CfgAclsFolderData.m_it;
		m_CfgAclsFolderData.m_it ++ )
	{
		pstAclsDataGroup = m_CfgAclsFolderData.m_it;
		if ( pstAclsDataGroup )
		{
	...
	m_proBar.SetPos( m_CfgAclsFolderData.m_vcAclsGroupList.size() );
	//	...
	m_proBar.ShowWindow( SW_HIDE );
}

VOID CTabSecurityCfgACLSFolder::DetectAclsProblems()
{
	INT  i;
	INT  nIndex;
	STPARSECFGDATAGROUP * pstAclsDataGroup;
...
//	ACCESS_MASK Mask;
	...
	for ( i = 0; i < nCount; i ++ )
	{
		...
		if ( nIndex >= 0 && nIndex < m_CfgAclsFolderData.m_vcAclsGroupList.size() )
		{
			m_CfgAclsFolderData.m_it = & m_CfgAclsFolderData.m_vcAclsGroupList.at( nIndex );
			pstAclsDataGroup = m_CfgAclsFolderData.m_it;
			if ( pstAclsDataGroup )
			{
...
unsigned __stdcall CTabSecurityCfgACLSFolder::_threadFixAclsProblems( PVOID arglist )
{
	if ( NULL == arglist )
	{
		return 1;
	...
	__try
	{
		CTabSecurityCfgACLSFolder * pThis = (CTabSecurityCfgACLSFolder*)arglist;
		if ( pThis )
		{
...
VOID CTabSecurityCfgACLSFolder::FixAclsProblemsProc()
{
	//	...
...
VOID CTabSecurityCfgACLSFolder::FixAclsProblems()
{
	INT  i;
	INT  nIndex;
	STPARSECFGDATAGROUP * pstAclsDataGroup;
