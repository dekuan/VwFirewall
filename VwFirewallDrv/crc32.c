...
unsigned long crc32_autolwr_a( const unsigned char * buf, unsigned int len )
{
	unsigned char cChar;
	unsigned int i;
	unsigned long crc32val;
	...
	for ( i = 0;  i < len;  i ++ )
	{
		...
		crc32val = crc32_tab[ ( crc32val ^ cChar ) & 0xff ] ^ ( crc32val >> 8 );
	}
	return crc32val;
}

unsigned long crc32_autolwr_w( const unsigned short * buf, unsigned int len )
{
	//
	//	typedef unsigned short wchar_t
	//	typedef wchar_t WCHAR
	//	buf	- wide char buffer
	//	len	- in btyes
	//
